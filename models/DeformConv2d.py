...
class DeformConv2d(nn.Module):
    ...
    def _get_p_n(self, N, dtype):
        p_n_x, p_n_y = torch.meshgrid(
            torch.arange(-(self.kernel_size-1)//2, (self.kernel_size-1)//2+1).to(device),
            torch.arange(-(self.kernel_size-1)//2, (self.kernel_size-1)//2+1).to(device))
        # (2N, 1)
    ...
    def _get_p_0(self, h, w, N, dtype):
        p_0_x, p_0_y = torch.meshgrid(
            torch.arange(1, h*self.stride+1, self.stride).to(device),
            torch.arange(1, w*self.stride+1, self.stride).to(device))
