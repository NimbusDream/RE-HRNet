...
from models.modules import BasicBlock, Bottleneck, SEBasicBlock, SEBottleneck, DeformBottleneck, CondBottleneck, CondBasicBlock
from models.GhostNet import GhostBottleneck, GhostModule
...
class HRNet(nn.Module):
    def __init__(self, c=48, nof_joints=17, bn_momentum=0.1):
        ...
        self.refine_net = refineNet((96, 72), nof_joints) # (96, 72) 或 （64, 48）

        # PRM Layer
        self.prm_layer = PRM(c)
    ...
    def forward(self, x):
        ...
        x = self.stage4(x)
        hrnet_out = self.prm_layer(x[0])
        hrnet_out = self.final_layer(hrnet_out)
        '''
        print(len(x))
        print(x[0].size())
        print(x[1].size())
        ...
        refine_out = self.refine_net(x)

        # x = self.final_layer(x)

        return hrnet_out, refine_out
