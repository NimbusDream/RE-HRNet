...
from datasets.CrowdPose import CrowdPoseDataset
from training.COCO import COCOTrain
from training.CrowdPose import CrowdPoseTrain
...
def main(exp_name,
         ...
         crowdpose_root_path="./datasets/crowdpose",
         crowdpose_bbox_path=None,
         seed=1,
         device=None):
    ...
    ds_train = CrowdPoseDataset(
        root_path=crowdpose_root_path, image_path="images", data_version="train", is_train=True, use_gt_bboxes=True, bbox_path="",
        image_width=image_resolution[1], image_height=image_resolution[0], color_rgb=True,
    )


    ds_val = CrowdPoseDataset(
        root_path=crowdpose_root_path, image_path="images", data_version="val", is_train=False, use_gt_bboxes=(crowdpose_bbox_path is None),
        bbox_path=crowdpose_bbox_path, image_width=image_resolution[1], image_height=image_resolution[0], color_rgb=True,
    )

    train = CrowdPoseTrain(
        exp_name=exp_name,
        ds_train=ds_train,
        ds_val=ds_val,
        epochs=epochs,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--exp_name", "-n",
                        ...
                        type=str, default="crowdpose/" + str(datetime.now().strftime("%Y%m%d_%H%M")))
    ...
    parser.add_argument("--crowdpose_root_path", help="COCO dataset root path", type=str, default="./datasets/crowdpose")
    parser.add_argument("--crowdpose_bbox_path", help="path of detected bboxes to use during evaluation",
                        type=str, default=None)
