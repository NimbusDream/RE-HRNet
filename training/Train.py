...
class Train(object):
    ...
    def __init__(self,
        ...
        self.refine_loss = JointsOHKMMSELoss().to(self.device)
        ...
        self.mean_acc_val = 0.
        self.mean_mAP_val = 0.
        self.hrnet_loss_record_train = 0.
        self.refine_loss_record_train = 0.
        self.hrnet_loss_record_val = 0.
        self.refine_loss_record_val = 0.
    ...
    def _train(self):
        ...
        for step, (image, target, target_weight, joints_data) in enumerate(tqdm(self.dl_train, desc='Training')):
            ...
            accs, avg_acc, cnt, joints_preds, joints_target = self.ds_train.evaluate_accuracy(refine_output, target)
        ...
        self.hrnet_loss_record_train /= len(self.dl_val)
        self.refine_loss_record_train /= len(self.dl_val)
        self.mean_loss_train /= len(self.dl_train)
        self.mean_acc_train /= len(self.dl_train)

        print('\nTrain: Loss %f - HRNet Loss %f - Refine Loss %f - Accuracy %f' % (self.mean_loss_train,
            self.hrnet_loss_record_train, self.refine_loss_record_train, self.mean_acc_train))
    ...
    def _val(self):
        ...
        with torch.no_grad():
            for step, (image, target, target_weight, joints_data) in enumerate(tqdm(self.dl_val, desc='Validating')):
                ...
                self.hrnet_loss_record_val += hrnet_loss.data.item()
                self.refine_loss_record_val += refine_loss.data.item()
        ...
        print('\nValidation: Loss %f - HRNet Loss %f - Refine Loss %f - Accuracy %f' % (self.mean_loss_val,
                    self.hrnet_loss_record_val, self.refine_loss_record_val, self.mean_acc_val))
